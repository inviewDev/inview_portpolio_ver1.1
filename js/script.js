...
function drag(e) {
  ...
  gsap.to(".ring_box", {
    ...
    onUpdate: () => {
      gsap.set(".cont_box", { backgroundPosition: (i) => getBgPos(i) });
    },
  });
  xPos = Math.round(e.clientX);
}
...
function getBgPos(i) {
  return (
    ...
    (gsap.utils.wrap(
      0,
      360,
      gsap.getProperty(".ring_box", "rotationY") - 180 - i * 36
    ) /
      360) *
      500 +
    "px 0px"
...
// parallax item
gsap.utils
  .toArray(".parallax_item_p, .parallax_item_span, .parallax_item_img")
  .forEach((item) => {
    let yPercentValue;
    let opacityValue;
...
$(document).ready(function () {
  ...
  $(".tap_btn_group a").on("click", function (e) {
    ...
    var ddd = parentTapBox
      .find(".tap_cont_group div")
      .eq(tapVal)
      .find("li").length;
    console.log(ddd);
  });
